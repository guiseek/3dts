...
export class OrbitControls extends EventDispatcher {
  ...
  constructor(public object: any, public domElement: HTMLElement) {
    super()

    this.position0 = object
    ...
    function handleMouseDownDolly(event: { clientX: number; clientY: number }) {
    ...
    function handleMouseMoveDolly(event: { clientX: number; clientY: number }) {
    ...
    function handleMouseWheel(event: any & { deltaY: number }) {
      if (event.deltaY < 0) {
        dollyIn(getZoomScale())
    ...
    function handleTouchMoveRotate(event: { pageX: number; pageY: number }) {
      if (pointers.length == 1) {
    ...
    function onPointerDown(event: { pointerId: number; pointerType: string }) {
    ...
    function onPointerUp(event: { pointerId: number }) {
      removePointer(event)
    ...
    function onMouseDown(
      event: any & { button: any; ctrlKey: any; metaKey: any; shiftKey: any }
    ) {
      let mouseAction
